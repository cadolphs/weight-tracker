...
Y-axis floor delta (`y-axis-floor`, DESIGN 2026-09-04, ADR-012): the shared chart engine guarantees a **minimum visible y-span of 2.0 kg** with both bounds snapped outward to a **0.5 kg grid**, on both surfaces and both lenses at every scale, so a plateaued month reads as a calm near-flat stroke instead of re-amplified noise (`js-2-judge`) — **zero new ports, adapters, containers, routes, assets, dependencies, external origins, or C4 changes**. The rule is a **pure Domain Core function**, `core/axis.py: y_axis_range(values) -> AxisRange | None` (constants `FLOOR_KG = 2.0`, `GRID_KG = 0.5`, `AUTO_PAD_FRACTION = 0.1` defined there and nowhere else): no values ⇒ none; span < 2.0 ⇒ `[mid − 1.0, mid + 1.0]`; span ≥ 2.0 ⇒ data min/max padded by 10 % of the span each side (an explicit formula, identical to uPlot's default after the snap for spans in [2, 10) kg, ≤ 0.5 kg apart above that); then `⌊2x + ε⌋/2` / `⌈2x − ε⌉/2` so every bound is an exact half-integer float on the wire. `GET /entries` and `GET /trend` each gain one **additive, optional** field `y_range: [lo, hi] | null` projected from the exact windowed series they already return — route-level enrichment on the `glance`/`recent` precedent; `WeightHistory` / `TrendProjection` gain **no methods at all**, ADR-004's series is byte-identical (G-3), and both reads stay pure (ADR-009). `graph.js` hands the pair to uPlot's `scales.y.range` behind a shape guard (array of two finite numbers, `lo < hi`) and otherwise omits the override so uPlot's own range renders — an imperfect axis, never a blank chart; the engine carries no numeric literal for the rule, so parity is structural (one core definition, one engine consumer, both surfaces fetch the same URLs). Placement was decided on testability: the suite has no browser and the repository no JS runner, so the @property honesty scenario is a Hypothesis test over the pure function rather than a source-regex on JavaScript, and the two taste constants (OQ-12/OQ-14, retunable after dogfood) sit inside the per-feature mutation gate. No new driven adapter ⇒ **no new Earned-Trust probes**; the one newly trusted input at the engine, the server's `y_range`, is contained by the shape guard. `graph.js` is pre-cached by the service worker, so DELIVER bumps `SHELL_CACHE` `-v4 → -v5`. DISCUSS assumption A28 ("the pre-feature auto-range") is refined to the explicit 10 % pad formula in the feature delta's Changed Assumptions.
...
| [ADR-012](adr-012-y-axis-range-rule.md) | Y-axis range rule = pure core `y_axis_range` (2.0 kg floor, 0.5 kg outward snap, explicit 10 % auto pad) served as additive `y_range` on `/entries` + `/trend`; engine applies, computes nothing |
